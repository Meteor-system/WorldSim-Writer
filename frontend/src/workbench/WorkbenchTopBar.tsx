...
export function WorkbenchTopBar({
  ...
  const showCreateWorld = createOpen ?? uncontrolledCreateOpen;
  const setShowCreateWorld = (open: boolean) => {
  ...
  return (
    ...
    <header className="workbench-topbar" role="banner">
      <div className="workbench-topbar-start">
        ...
        >
          <option value="">{currentWorldId === null ? '返回书架' : '当前世界'}</option>
          {worlds.map((item) => (
      ...
      <div className="workbench-topbar-end text-sm ink-muted">
        {currentWorldId !== null && <span>v{currentWorldVersion}</span>}
        {currentWorldId !== null && <span>{currentWorldStatus === 'archived' ? '已归档' : '连载中'}</span>}
