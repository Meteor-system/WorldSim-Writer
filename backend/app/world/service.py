    ...
    return [
        {
            ...
            'content': (
                ...
                '严格 JSON 契约：顶层必须且只能有 draft、first_chapter_goal、generation_notes、safety_notes、followup_questions 五个字段；'
                'draft 必须且只能有 title、genre_template、truth_canon、tone_profile、starter_assets 五个字段，并完全符合 WorldCreateRequest。'
                'starter_assets.characters 的每个角色必须提供 name、role_type、status、public_profile、hidden_traits、destiny_flag、current_goals，且至少 2 个角色。'
                'starter_assets.relations 的每条关系必须提供 source_index、target_index、relation_type、intensity、visibility；索引为 0-based、不得相同，intensity 为 1..5。'
                'starter_assets.foreshadows 的每条伏笔必须提供 title、description、foreshadow_type、status、urgency_level、related_character_indexes、expected_resolution_window；status 优先 planted，urgency_level 为 1..5。'
                'relations 和 foreshadows 使用 source_index、target_index、related_character_indexes 引用 characters 数组索引。'
                'first_chapter_goal 必须为非空字符串；generation_notes、safety_notes、followup_questions 必须为字符串数组。'
                '只返回一个 JSON 对象；不得返回 Markdown、代码围栏或解释。'
