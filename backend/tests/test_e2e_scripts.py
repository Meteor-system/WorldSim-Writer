...
def test_e2e_smoke_script_requires_preview_character_changes_to_be_list_of_objects(monkeypatch):
    monkeypatch.setenv('BASE_URL', 'https://worldsim.test')
    module = load_e2e_smoke_module()
    transport = SequencedTransport(
        [
    ...
    assert summary['failed_step'] == 'approval_preview'
    assert summary['error'] == 'INVALID_FIELD_TYPES'
    assert summary['invalid_fields'] == ['character_changes']
    assert [request.url.path for request in transport.requests] == [
        '/health',
        '/auth/register',
        '/worlds/from-template',
...
def test_e2e_smoke_script_requires_preview_foreshadow_changes_to_be_list_of_objects(monkeypatch):
    monkeypatch.setenv('BASE_URL', 'https://worldsim.test')
    module = load_e2e_smoke_module()
    transport = SequencedTransport(
        [
    ...
    assert summary['failed_step'] == 'approval_preview'
    assert summary['error'] == 'INVALID_FIELD_TYPES'
    assert summary['invalid_fields'] == ['foreshadow_changes']
    assert [request.url.path for request in transport.requests] == [
        '/health',
        '/auth/register',
        '/worlds/from-template',
...
def test_e2e_smoke_script_requires_events_summary_to_be_object_when_present(monkeypatch):
    monkeypatch.setenv('BASE_URL', 'https://worldsim.test')
    module = load_e2e_smoke_module()
    transport = SequencedTransport(
        [
            ...
            json_response({'id': 20, 'status': 'approved', 'approved_version': 2}),
            overview_response(),
            json_response({'items': [{'event_type': 'world_version_increment'}], 'summary': [{'event_type_counts': {'chapter_approved': 1}}]}),
        ]
    )
    client = httpx.Client(transport=transport, base_url='https://worldsim.test')
    ...
    assert summary['failed_step'] == 'events'
    assert summary['error'] == 'INVALID_FIELD_TYPES'
    assert summary['invalid_fields'] == ['summary']
    assert [request.url.path for request in transport.requests] == [
        '/health',
        '/auth/register',
        '/worlds/from-template',
...
def test_e2e_smoke_script_requires_markdown_export_archive_base64_to_be_string(monkeypatch):
    monkeypatch.setenv('BASE_URL', 'https://worldsim.test')
    module = load_e2e_smoke_module()
    transport = SequencedTransport(
        [
    ...
    assert summary['failed_step'] == 'markdown_export'
    assert summary['error'] == 'INVALID_FIELD_TYPES'
    assert summary['invalid_fields'] == ['archive_base64']
    assert [request.url.path for request in transport.requests] == [
        '/health',
        '/auth/register',
        '/worlds/from-template',
...
def test_e2e_smoke_script_fails_when_markdown_export_archive_format_is_invalid(monkeypatch):
    monkeypatch.setenv('BASE_URL', 'https://worldsim.test')
    module = load_e2e_smoke_module()
    transport = SequencedTransport(
        [
...
def test_e2e_smoke_script_fails_when_markdown_export_world_file_is_missing(monkeypatch):
    monkeypatch.setenv('BASE_URL', 'https://worldsim.test')
    module = load_e2e_smoke_module()
    transport = SequencedTransport(
        [
...
def test_e2e_smoke_script_requires_readiness_status_to_be_string(monkeypatch):
    monkeypatch.setenv('BASE_URL', 'https://worldsim.test')
    module = load_e2e_smoke_module()
    transport = SequencedTransport(
        [
    ...
    assert summary['failed_step'] == 'approval_readiness'
    assert summary['error'] == 'INVALID_FIELD_TYPES'
    assert summary['invalid_fields'] == ['status']
    assert [request.url.path for request in transport.requests] == [
        '/health',
        '/auth/register',
        '/worlds/from-template',
...
def test_e2e_smoke_script_requires_readiness_status_to_be_known(monkeypatch):
    monkeypatch.setenv('BASE_URL', 'https://worldsim.test')
    module = load_e2e_smoke_module()
    transport = SequencedTransport(
        [
    ...
    assert summary['invalid_fields'] == ['status']
    assert summary['allowed_values'] == {'status': ['blocked', 'needs_review', 'ready']}
    assert [request.url.path for request in transport.requests] == [
        '/health',
        '/auth/register',
        '/worlds/from-template',
...
def test_e2e_smoke_script_requires_readiness_blocking_reasons_to_be_list(monkeypatch):
    monkeypatch.setenv('BASE_URL', 'https://worldsim.test')
    module = load_e2e_smoke_module()
    transport = SequencedTransport(
        [
    ...
    assert summary['failed_step'] == 'approval_readiness'
    assert summary['error'] == 'INVALID_FIELD_TYPES'
    assert summary['invalid_fields'] == ['blocking_reasons']
    assert [request.url.path for request in transport.requests] == [
        '/health',
        '/auth/register',
        '/worlds/from-template',
...
def test_e2e_smoke_script_requires_readiness_warnings_to_be_list(monkeypatch):
    monkeypatch.setenv('BASE_URL', 'https://worldsim.test')
    module = load_e2e_smoke_module()
    transport = SequencedTransport(
        [
    ...
    assert summary['failed_step'] == 'approval_readiness'
    assert summary['error'] == 'INVALID_FIELD_TYPES'
    assert summary['invalid_fields'] == ['warnings']
    assert [request.url.path for request in transport.requests] == [
        '/health',
        '/auth/register',
        '/worlds/from-template',
...
def test_e2e_smoke_script_requires_consistency_blocking_count_before_approval(monkeypatch):
    monkeypatch.setenv('BASE_URL', 'https://worldsim.test')
    module = load_e2e_smoke_module()
    transport = SequencedTransport(
        [
    ...
    assert summary['failed_step'] == 'approval_consistency'
    assert summary['error'] == 'MISSING_REQUIRED_FIELDS'
    assert summary['missing_fields'] == ['consistency_summary.blocking_count']
    assert [request.url.path for request in transport.requests] == [
        '/health',
        '/auth/register',
        '/worlds/from-template',
...
def test_e2e_smoke_script_requires_consistency_blocking_count_to_be_int(monkeypatch):
    monkeypatch.setenv('BASE_URL', 'https://worldsim.test')
    module = load_e2e_smoke_module()
    transport = SequencedTransport(
        [
    ...
    assert summary['failed_step'] == 'approval_consistency'
    assert summary['error'] == 'INVALID_FIELD_TYPES'
    assert summary['invalid_fields'] == ['consistency_summary.blocking_count']
    assert [request.url.path for request in transport.requests] == [
        '/health',
        '/auth/register',
        '/worlds/from-template',
...
def test_e2e_smoke_script_requires_consistency_blocking_count_to_be_non_negative(monkeypatch):
    monkeypatch.setenv('BASE_URL', 'https://worldsim.test')
    module = load_e2e_smoke_module()
    transport = SequencedTransport(
        [
...
def test_e2e_smoke_script_requires_consistency_warnings_before_approval(monkeypatch):
    monkeypatch.setenv('BASE_URL', 'https://worldsim.test')
    module = load_e2e_smoke_module()
    transport = SequencedTransport(
        [
    ...
    assert summary['failed_step'] == 'approval_consistency'
    assert summary['error'] == 'MISSING_REQUIRED_FIELDS'
    assert summary['missing_fields'] == ['consistency_warnings']
    assert [request.url.path for request in transport.requests] == [
        '/health',
        '/auth/register',
        '/worlds/from-template',
...
def test_e2e_smoke_script_requires_consistency_warnings_to_be_list_of_objects(monkeypatch):
    monkeypatch.setenv('BASE_URL', 'https://worldsim.test')
    module = load_e2e_smoke_module()
    transport = SequencedTransport(
        [
    ...
    assert summary['failed_step'] == 'approval_consistency'
    assert summary['error'] == 'INVALID_FIELD_TYPES'
    assert summary['invalid_fields'] == ['consistency_warnings']
    assert [request.url.path for request in transport.requests] == [
        '/health',
        '/auth/register',
        '/worlds/from-template',
...
def test_e2e_smoke_script_reports_invalid_json_response_body_for_success_response(monkeypatch):
    ...
    module = load_e2e_smoke_module()
    invalid_body = (
        'not json api_key=sk-invalid-json '
    ...
    transport = SequencedTransport(
        [
            ...
            json_response({'access_token': 'token', 'user': {'id': 1, 'email': 'e2e-smoke@example.com'}}),
            json_response({'id': 10, 'world_version': 1}),
            httpx.Response(200, text=invalid_body),
        ]
    )
    client = httpx.Client(transport=transport, base_url='https://worldsim.test')
    ...
    assert 'sk-invalid-json' not in response_body
...
def test_e2e_smoke_script_reports_non_object_json_response_body_for_success_response(monkeypatch):
    monkeypatch.setenv('BASE_URL', 'https://worldsim.test')
    module = load_e2e_smoke_module()
    transport = SequencedTransport(
        [
    ...
    assert summary['status_code'] == 200
    assert 'sk-array-secret' not in response_body
    assert 'https://provider.example/v1' not in response_body
    assert 'secret-array-model' not in response_body
    assert '[REDACTED_SECRET]' in response_body
    assert '[REDACTED_URL]' in response_body
    assert '[REDACTED_MODEL]' in response_body
...
def test_e2e_smoke_script_redacts_json_authorization_response_body_details(monkeypatch):
    monkeypatch.setenv('BASE_URL', 'https://worldsim.test')
    module = load_e2e_smoke_module()
    sensitive_body = (
        '{"error":"MODEL_REQUEST_FAILED",'
    ...
    assert summary['failed_step'] == 'draft'
    assert summary['status_code'] == 502
    assert 'json-secret-token' not in response_body
    assert 'second-json-secret' not in response_body
    assert response_body.count('[REDACTED_SECRET]') == 2
...
def test_e2e_smoke_script_requires_health_flags_to_be_booleans(monkeypatch):
    monkeypatch.setenv('BASE_URL', 'https://worldsim.test')
    module = load_e2e_smoke_module()
    transport = SequencedTransport(
        [
