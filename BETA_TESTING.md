...
- When continuous smoke is enabled, `checks.continuous_chapters.previous_chapter_summary_present`, `second_context_goal_matches`, `second_previous_chapter_summary_present`, `fresh_context_goal_matches`, `fresh_previous_chapter_summary_present`, `stale_draft_rejected`, `second_chapter_approved`, and `approved_chapter_count_incremented` should be `true`; `priority_foreshadow_count`, `second_priority_foreshadow_count`, `fresh_priority_foreshadow_count`, and `second_proposed_change_count` should be greater than `0`; `second_preview_version_conflict`, `second_readiness_blocked`, and `second_consistency_blocked` should be `false`; `second_readiness_status` should be `ready` or `needs_review`; `second_consistency_status` should be `clear` or `needs_review`; and `second_draft_source_world_version`, `stale_world_version`, `fresh_prep_world_version`, `fresh_draft_source_world_version`, and `expected_second_world_version_after` should show the continuous world-version checkpoints. Failures stop at `failed_step` values such as `next_chapter_prep`, `second_draft`, `stale_draft_approval`, `fresh_next_chapter_prep`, `fresh_second_draft`, `second_approval_preview`, `second_approval_readiness`, `second_approval_consistency`, or `second_overview` with explicit errors including `PREVIOUS_CHAPTER_SUMMARY_MISSING`, `PRIORITY_FORESHADOWS_MISSING`, `EXECUTION_CONTEXT_NOT_CURRENT`, `EXECUTION_CONTEXT_FORESHADOWS_MISSING`, `STALE_DRAFT_APPROVAL_NOT_REJECTED`, `APPROVAL_PREVIEW_BLOCKED`, `NO_PROPOSED_PROJECTION_CHANGES`, `APPROVAL_READINESS_BLOCKED`, or `APPROVAL_CONSISTENCY_BLOCKED`.
