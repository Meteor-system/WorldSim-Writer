...
- When continuous smoke is enabled, `checks.continuous_chapters.previous_chapter_summary_present`, `stale_draft_rejected`, `second_chapter_approved`, and `approved_chapter_count_incremented` should be `true`, `priority_foreshadow_count` should be greater than `0`, and `fresh_draft_source_world_version` should match the latest world version after the stale-draft setup edit. Failures stop at `failed_step` values such as `next_chapter_prep`, `second_draft`, `stale_draft_approval`, `fresh_second_draft`, or `second_overview` with explicit errors including `PREVIOUS_CHAPTER_SUMMARY_MISSING`, `PRIORITY_FORESHADOWS_MISSING`, `EXECUTION_CONTEXT_NOT_CURRENT`, `EXECUTION_CONTEXT_FORESHADOWS_MISSING`, or `STALE_DRAFT_APPROVAL_NOT_REJECTED`.
