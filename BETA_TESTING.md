...
- `checks.health.migration_up_to_date` is `true`; if the smoke JSON stops at `failed_step: "health"` with `error: "MIGRATION_NOT_UP_TO_DATE"`, run `python scripts/run_migrations.py` from `backend/`, restart the backend, and rerun smoke. This is the release-safe wrapper around `alembic upgrade head`; run it as one serialized job, never once per API worker.
