...
- **工作台**：全屏三栏、正文分期、检查器页签、选区润色/重写
- **记忆卡**：草稿展示本章 memory_card；检查器可预览下一章记忆检索
- **收束计划**：熵/弧光模式/收束比展示；逾期项可写入空的章节目标

### 2.4 角色管理
- 创建 / 查看 / 更新 / 删除角色
- 角色性别（男/女/其他/未定），用于代词约束
- 角色列表与筛选
- 角色关系管理（创建/查看/更新/删除关系）
- **真相层**：世界概览可编辑未冻结层；冻结层只读；写入提升世界版本
...
### 2.10 质量引擎与文学评审
- 章节质量报告 (ChapterQualityReport)：钩子、伏笔压力、角色一致、情绪曲线、节奏、章节目标 6 项检查
- 文学性独立评审 (LiteraryReviewReport)：解释过度、情绪直给、桥段俗套、对白功能化、结尾钩子 5 维评分
- 前端"质量/报告"页签展示报告；可选 LLM 失败时静默降级，不阻塞写作流程

### 2.11 叙事收束系统 (Narrative Convergence)
- 叙事熵、Arc Mode（扩张/整理/加压/收束/兑现/终局）、收束比、收束计划
- 伏笔紧迫度自动升级：剩余章节不足时提高优先级
- 每 10 章生成卷末过渡报告并写入事件日志
- 前端"收束"页签展示熵、阶段、收束计划与逾期线索
